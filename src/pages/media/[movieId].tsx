          ...
          <Paper sx={{ marginBottom: "2rem" }}>
            <Grid container>
              <Grid item md={4} sm={5} xs={12} alignItems='center'>
                <img alt={`${movieDetails.Title} box office poster`} src={movieDetails.Poster} style={{ objectFit: "cover", width: "100%", height: "100%" }} />
              </Grid>
            ...
            <Paper sx={{ padding: "1.5rem" }}>
              ...
              {Array(Number(seriesData?.totalSeasons))
                ...
                .map((item, index) => {
                  ...
                  return (
                    <SeasonAccordian key={`seasion-accordian-${index}`} title={`Season ${index + 1}`} seasonNumber={index + 1} seriesId={movieDetails.imdbID} />
                  );
                })}
            </Paper>
          )}
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  try {
    const { data } = await axios.get<MovieDetails | OMDBErrorResponse>(`${process.env.OMDB_URL}?i=${movieId}&plot=full&apikey=${process.env.OMDB_APIKEY}`);
    ...
    if (data.Type === "series") {
      const { data } = await axios.get<SeriesData | OMDBErrorResponse>(`${process.env.OMDB_URL}?i=${movieId}&Season=1&apikey=${process.env.OMDB_APIKEY}`);
